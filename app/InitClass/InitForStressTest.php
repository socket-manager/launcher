 ...
 */
class InitForStressTest implements IInitSocketManager
{
    //--------------------------------------------------------------------------
    // 定数
    //--------------------------------------------------------------------------
    ...
    public function getLogWriter()
    {
        return function(string $p_level, array $p_param)
        {
            ...
            error_log($log, 3, "./logs/socket-manager/stress-test/{$filename}_C{$this->port}.log");
        };
    }
    ...
    {
        return function(ParameterForStressTest $p_param, $p_dat): ?string
        {
            /**
