...
use App\UnitParameter\ParameterForBenchmark;
 ...
 */
class ProtocolForBenchmark implements IEntryUnits
{
    //--------------------------------------------------------------------------
    // 定数
    //--------------------------------------------------------------------------
    ...
    public function getUnitList(string $p_que): array
    {
        ...
        {
            $ret[] = [
                'status' => ProtocolForBenchmarkStatusEnum::START->value,
                'unit' => $this->getAcceptStart()
            ];
            $ret[] = [
                'status' => ProtocolForBenchmarkStatusEnum::CREATE->value,
                'unit' => $this->getAcceptCreate()
            ];
            $ret[] = [
                'status' => ProtocolForBenchmarkStatusEnum::SEND->value,
        ...
        if($p_que === ProtocolQueueEnum::RECV->value)
        {
            ...
            $ret[] = [
                'status' => ProtocolForBenchmarkStatusEnum::LENGTH->value,
                'unit' => $this->getRecvLength()
            ];
            $ret[] = [
                'status' => ProtocolForBenchmarkStatusEnum::MASK->value,
                'unit' => $this->getRecvMask()
            ];
            $ret[] = [
                'status' => ProtocolForBenchmarkStatusEnum::PAYLOAD->value,
                'unit' => $this->getRecvPayload()
            ];
            $ret[] = [
                'status' => ProtocolForBenchmarkStatusEnum::PONG_CREATE->value,
                'unit' => $this->getRecvPongCreate()
            ];
            $ret[] = [
                'status' => ProtocolForBenchmarkStatusEnum::PONG_SENDING->value,
        ...
        if($p_que === ProtocolQueueEnum::SEND->value)
        {
            ...
            $ret[] = [
                'status' => ProtocolForBenchmarkStatusEnum::SENDING->value,
                'unit' => $this->getSendSending()
            ];
        }
        else
        ...
        if($p_que === ProtocolQueueEnum::ALIVE->value)
        {
            ...
            $ret[] = [
                'status' => ProtocolForBenchmarkStatusEnum::RECV->value,
                'unit' => $this->getRecvStart()
            ];
            $ret[] = [
                'status' => ProtocolForBenchmarkStatusEnum::LENGTH->value,
     ...
     * 処理名：受信
     * 
     * @param ParameterForBenchmark $p_param UNITパラメータ
     * @return ?string 遷移先のステータス名
     */
    protected function getAcceptStart()
    {
        return function(ParameterForBenchmark $p_param): ?string
        {
            // ソケット受信
            $rcv = '';
            ...
            if(strlen($rcv) > 0)
            {
                ...
                if(preg_match('/\r\n\r\n/', $buf, $matches))
                {
                    ...
                    $sta = $fnc($p_param);
                    if($sta === ProtocolForBenchmarkStatusEnum::START->value)
                    {
                        // NG判定
                        ...
                        return ProtocolForBenchmarkStatusEnum::SEND->value;
                    }
    ...
    protected function getAcceptCreate()
    {
        return function(ParameterForBenchmark $p_param): ?string
        {
            ...
            if(isset($hdrs['Sec-WebSocket-Version']))
            {
                ...
                if($ver == ParameterForBenchmark::CHAT_PROTOCOL_VERSION)
                {
                    $ok = true;
                }
                else
                {
                    ...
                    $res  =
                        ...
                        "Sec-WebSocket-Version: ".ParameterForBenchmark::CHAT_PROTOCOL_VERSION."\r\n\r\n";
                }
            }
            else
            {
            ...
            $sta = $fnc($p_param);
            if($sta === ProtocolForBenchmarkStatusEnum::CREATE->value)
            {
                return ProtocolForBenchmarkStatusEnum::SEND->value;
            }
            return $sta;
    ...
    protected function getAcceptSend()
    {
        return function(ParameterForBenchmark $p_param): ?string
        {
            ...
            if($hdrs['result'] === false)
            {
                ...
                // リトライカウント判定
                if($hdrs['retry'] >= ParameterForBenchmark::CHAT_HANDSHAKE_RETRY)
                {
                    // 強制切断
                ...
                return ProtocolForBenchmarkStatusEnum::START->value;
            }
    ...
    protected function getRecvStart()
    {
        return function(ParameterForBenchmark $p_param): ?string
        {
            ...
            // データ長種別を設定
            $len_knd = $dat_ary['chars2'] & ParameterForBenchmark::CHAT_PAYLOAD_LEN_MASK;
    
            $entry_data = array();
            if($len_knd == ParameterForBenchmark::CHAT_PAYLOAD_LEN_CODE_2)
            {
                $entry_data = [
                      'length_byte'  => 2
                    , 'length'       => null
                ...
                $sta = ProtocolForBenchmarkStatusEnum::LENGTH->value;
    
                // 受信サイズを設定
                $p_param->protocol()->setReceivingSize($entry_data['length_byte']);
            }
            elseif($len_knd == ParameterForBenchmark::CHAT_PAYLOAD_LEN_CODE_8)
            {
                $entry_data = [
                      'length_byte'  => 8
                    , 'length'       => null
            ...
            else
            {
                ...
                $sta = ProtocolForBenchmarkStatusEnum::MASK->value;
    ...
    protected function getRecvLength()
    {
        return function(ParameterForBenchmark $p_param): ?string
        {
            ...
            return ProtocolForBenchmarkStatusEnum::MASK->value;
        };
    }
    ...
    protected function getRecvMask()
    {
        return function(ParameterForBenchmark $p_param): ?string
        {
            ...
            return ProtocolForBenchmarkStatusEnum::PAYLOAD->value;
        };
    }
    ...
    protected function getRecvPayload()
    {
        return function(ParameterForBenchmark $p_param): ?string
        {
            ...
            if(($entry_data['first_byte'] & 0x0f) === ParameterForBenchmark::CHAT_OPCODE_CLOSE_MASK)
            {
                // 切断パラメータを取得
            ...
            // PONGの場合は抜ける
            if(($entry_data['first_byte'] & 0x0f) === ParameterForBenchmark::CHAT_OPCODE_PONG_MASK)
            {
                return null;
            }
            else
            // PINGの場合はPONGを返す
            if(($entry_data['first_byte'] & 0x0f) === ParameterForBenchmark::CHAT_OPCODE_PING_MASK)
            {
                $p_param->setTempBuff(['recv_buff' => $entry_data]);
                return ProtocolForBenchmarkStatusEnum::PONG_CREATE->value;
            }
    ...
    protected function getRecvPongCreate()
    {
        return function(ParameterForBenchmark $p_param): ?string
        {
            ...
            $entry_data = $p_param->getTempBuff(['recv_buff']);
            $entry_data = $entry_data['recv_buff'];
            $b1 = ParameterForBenchmark::CHAT_FIN_BIT_MASK | (ParameterForBenchmark::CHAT_OPCODE_PONG_MASK & 0x0f);  // maskなし
            ...
            {
                $header = pack('CCn', $b1, ParameterForBenchmark::CHAT_PAYLOAD_LEN_CODE_2, $length);
            }
            //データ長８バイト
            else
            if($length >= 65536)
            {
                $header = pack('CCNN', $b1, ParameterForBenchmark::CHAT_PAYLOAD_LEN_CODE_8, $length);
            }
        
            $p_param->protocol()->setSendingData($header.$entry_data['data']);

            return ProtocolForBenchmarkStatusEnum::PONG_SENDING->value;
        };
    }
    ...
    protected function getSendStart()
    {
        return function(ParameterForBenchmark $p_param): ?string
        {
            ...
            $b1 = ParameterForBenchmark::CHAT_FIN_BIT_MASK | (ParameterForBenchmark::CHAT_OPCODE_TEXT_MASK & 0x0f);  // maskなし
            ...
            return ProtocolForBenchmarkStatusEnum::SENDING->value;
        };
    }
    ...
    protected function getCloseStart()
    {
        return function(ParameterForBenchmark $p_param): ?string
        {
            ...
            $b1 = ParameterForBenchmark::CHAT_FIN_BIT_MASK | (ParameterForBenchmark::CHAT_OPCODE_CLOSE_MASK & 0x0f);  // maskなし
            $length = strlen($payload);
    ...
    protected function getAliveStart()
    {
        return function(ParameterForBenchmark $p_param): ?string
        {
            ...
            $b1 = ParameterForBenchmark::CHAT_FIN_BIT_MASK | (ParameterForBenchmark::CHAT_OPCODE_PING_MASK & 0x0f);  // maskなし
            $length = strlen($payload);
    ...
    protected function getAliveSending()
    {
        return function(ParameterForBenchmark $p_param): ?string
        {
            ...
            return ProtocolForBenchmarkStatusEnum::RECV->value;
        };
    }
