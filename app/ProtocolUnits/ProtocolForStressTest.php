 ...
 */
class ProtocolForStressTest extends ProtocolForBenchmark
{
    //--------------------------------------------------------------------------
    // 定数
    //--------------------------------------------------------------------------
     ...
     * 処理名：データ作成（ハンドシェイク用）
     * 
     * @param ParameterForStressTest $p_param UNITパラメータ
    ...
    protected function getConnectStart()
    {
        return function(ParameterForStressTest $p_param): ?string
        {
            ...
            // ハンドシェイクデータの生成
            $ver = ParameterForStressTest::CHAT_PROTOCOL_VERSION;
            ...
            if(strlen($rcv) > 0)
            {
                ...
                if(preg_match('/\r\n\r\n/', $buf, $matches))
                {
                    ...
                    if($p_param->sample_count >= $p_param->samples)
                    {
                        ...
                        // 次回タイムアウトの設定
                        $p_param->next_timeout = $timer['timer']['end'] + ($p_param->sampling_interval * 1000000000);
    ...
    protected function getRecvStart()
    {
        return function(ParameterForStressTest $p_param): ?string
        {
            ...
            // データ長種別を設定
            $len_knd = $dat_ary['chars2'] & ParameterForStressTest::CHAT_PAYLOAD_LEN_MASK;
    
            $entry_data = array();
            if($len_knd == ParameterForStressTest::CHAT_PAYLOAD_LEN_CODE_2)
            {
                $entry_data = [
                      'length_byte'  => 2
                    , 'length'       => null
            ...
            elseif($len_knd == ParameterForStressTest::CHAT_PAYLOAD_LEN_CODE_8)
            {
                $entry_data = [
                      'length_byte'  => 8
                    , 'length'       => null
    ...
    protected function getRecvPayload()
    {
        return function(ParameterForStressTest $p_param): ?string
        {
            ...
            if(($entry_data['first_byte'] & 0x0f) === ParameterForStressTest::CHAT_OPCODE_CLOSE_MASK)
            {
                // 切断パラメータを取得
            ...
            // PONGの場合は抜ける
            if(($entry_data['first_byte'] & 0x0f) === ParameterForStressTest::CHAT_OPCODE_PONG_MASK)
            {
            ...
            // PINGの場合はPONGを返す
            if(($entry_data['first_byte'] & 0x0f) === ParameterForStressTest::CHAT_OPCODE_PING_MASK)
            {
                $p_param->setTempBuff(['recv_buff' => $entry_data]);
    ...
    protected function getRecvPongCreate()
    {
        return function(ParameterForStressTest $p_param): ?string
        {
            ...
            $entry_data = $p_param->getTempBuff(['recv_buff']);
            $entry_data = $entry_data['recv_buff'];
            $b1 = ParameterForStressTest::CHAT_FIN_BIT_MASK | (ParameterForStressTest::CHAT_OPCODE_PONG_MASK & 0x0f);  // maskあり
            ...
            {
                $header = pack('CCn', $b1, ParameterForStressTest::CHAT_PAYLOAD_LEN_CODE_2, $length);
            }
            //データ長８バイト
            else
            if($length >= 65536)
            {
                $header = pack('CCNN', $b1, ParameterForStressTest::CHAT_PAYLOAD_LEN_CODE_8, $length);
            }
    ...
    protected function getSendStart()
    {
        return function(ParameterForStressTest $p_param): ?string
        {
            ...
            $b1 = ParameterForStressTest::CHAT_FIN_BIT_MASK | (ParameterForStressTest::CHAT_OPCODE_TEXT_MASK & 0x0f);  // maskあり
            $mask = random_bytes(4);
    ...
    protected function getCloseStart()
    {
        return function(ParameterForStressTest $p_param): ?string
        {
            ...
            $b1 = ParameterForStressTest::CHAT_FIN_BIT_MASK | (ParameterForStressTest::CHAT_OPCODE_CLOSE_MASK & 0x0f);  // maskあり
            $mask = random_bytes(4);
            $payload = $this->maskPayload($payload, $mask);
            $length = strlen($payload);
    ...
    protected function getAliveStart()
    {
        return function(ParameterForStressTest $p_param): ?string
        {
            ...
            $b1 = ParameterForStressTest::CHAT_FIN_BIT_MASK | (ParameterForStressTest::CHAT_OPCODE_PING_MASK & 0x0f);  // maskあり
            $mask = random_bytes(4);
            $payload = $this->maskPayload($payload, $mask);
            $length = strlen($payload);
            ...
            if($length <= 125)
            {
                $header = pack('CC', $b1, $length | ~ParameterForStressTest::CHAT_PAYLOAD_LEN_MASK);
            }
            // データ長２バイト
            else
            if($length > 125 && $length < 65536)
            {
                $header = pack('CCn', $b1, ParameterForStressTest::CHAT_PAYLOAD_LEN_CODE_2 | ~ParameterForStressTest::CHAT_PAYLOAD_LEN_MASK, $length);
            }
            //データ長８バイト
            else
            if($length >= 65536)
            {
                $header = pack('CCNN', $b1, ParameterForStressTest::CHAT_PAYLOAD_LEN_CODE_8 | ~ParameterForStressTest::CHAT_PAYLOAD_LEN_MASK, $length);
            }
