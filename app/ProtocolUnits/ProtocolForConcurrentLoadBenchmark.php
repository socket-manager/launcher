 ...
 */
class ProtocolForConcurrentLoadBenchmark extends ProtocolForBenchmark
{
    //--------------------------------------------------------------------------
    // 定数
    //--------------------------------------------------------------------------
    ...
    public function getUnitList(string $p_que): array
    {
        ...
        {
            $ret[] = [
                'status' => ProtocolForConcurrentLoadBenchmarkStatusEnum::START->value,
                'unit' => $this->getConnectStart()
            ];
            $ret[] = [
                'status' => ProtocolForConcurrentLoadBenchmarkStatusEnum::SEND->value,
                'unit' => $this->getConnectSend()
            ];
            $ret[] = [
                'status' => ProtocolForConcurrentLoadBenchmarkStatusEnum::RECV->value,
        ...
        if($p_que === ProtocolQueueEnum::RECV->value)
        {
            ...
            $ret[] = [
                'status' => ProtocolForConcurrentLoadBenchmarkStatusEnum::LENGTH->value,
                'unit' => $this->getRecvLength()
            ];
            $ret[] = [
                'status' => ProtocolForConcurrentLoadBenchmarkStatusEnum::PAYLOAD->value,
                'unit' => $this->getRecvPayload()
            ];
            $ret[] = [
                'status' => ProtocolForConcurrentLoadBenchmarkStatusEnum::PONG_CREATE->value,
                'unit' => $this->getRecvPongCreate()
            ];
            $ret[] = [
                'status' => ProtocolForConcurrentLoadBenchmarkStatusEnum::PONG_SENDING->value,
        ...
        if($p_que === ProtocolQueueEnum::SEND->value)
        {
            ...
            $ret[] = [
                'status' => ProtocolForConcurrentLoadBenchmarkStatusEnum::SENDING->value,
                'unit' => $this->getSendSending()
            ];
        }
        else
     ...
     * 処理名：データ作成（ハンドシェイク用）
     * 
     * @param ParameterForConcurrentLoadBenchmark $p_param UNITパラメータ
     * @return ?string 遷移先のステータス名
     */
    protected function getConnectStart()
    {
        return function(ParameterForConcurrentLoadBenchmark $p_param): ?string
        {
            $host = $port = null;
            $p_param->getRemoteAddr($host, $port);

            // ハンドシェイクデータの生成
            $ver = ParameterForConcurrentLoadBenchmark::CHAT_PROTOCOL_VERSION;
            ...
            $start = hrtime(true);
            $p_param->setTempBuff(['timer' => ['start' => $start, 'end' => null]]);

            if($sta === ProtocolForConcurrentLoadBenchmarkStatusEnum::START->value)
            {
               return ProtocolForConcurrentLoadBenchmarkStatusEnum::SEND->value; 
            }
            return ProtocolForConcurrentLoadBenchmarkStatusEnum::RECV->value;
        };
    }
        ...
        return function(ParameterForConcurrentLoadBenchmark $p_param): ?string
        {
            ...
            if(strlen($rcv) > 0)
            {
                ...
                if(preg_match('/\r\n\r\n/', $buf, $matches))
                {
                    ...
                    $timer = $p_param->getTempBuff(['timer']);
                    $timer['timer']['end'] = hrtime(true);
    ...
    protected function getRecvStart()
    {
        return function(ParameterForConcurrentLoadBenchmark $p_param): ?string
        {
            ...
            // データ長種別を設定
            $len_knd = $dat_ary['chars2'] & ParameterForConcurrentLoadBenchmark::CHAT_PAYLOAD_LEN_MASK;
    
            $entry_data = array();
            if($len_knd == ParameterForConcurrentLoadBenchmark::CHAT_PAYLOAD_LEN_CODE_2)
            {
                $entry_data = [
                      'length_byte'  => 2
                    , 'length'       => null
                ...
                $sta = ProtocolForConcurrentLoadBenchmarkStatusEnum::LENGTH->value;
    
                // 受信サイズを設定
                $p_param->protocol()->setReceivingSize($entry_data['length_byte']);
            }
            elseif($len_knd == ParameterForConcurrentLoadBenchmark::CHAT_PAYLOAD_LEN_CODE_8)
            {
                $entry_data = [
                      'length_byte'  => 8
                    , 'length'       => null
            ...
            else
            {
                ...
                $sta = ProtocolForConcurrentLoadBenchmarkStatusEnum::PAYLOAD->value;
    ...
    protected function getRecvLength()
    {
        return function(ParameterForConcurrentLoadBenchmark $p_param): ?string
        {
            ...
            return ProtocolForConcurrentLoadBenchmarkStatusEnum::PAYLOAD->value;
        };
    }
    ...
    protected function getRecvPayload()
    {
        return function(ParameterForConcurrentLoadBenchmark $p_param): ?string
        {
            ...
            if(($entry_data['first_byte'] & 0x0f) === ParameterForConcurrentLoadBenchmark::CHAT_OPCODE_CLOSE_MASK)
            {
                // 切断パラメータを取得
            ...
            // PONGの場合は抜ける
            if(($entry_data['first_byte'] & 0x0f) === ParameterForConcurrentLoadBenchmark::CHAT_OPCODE_PONG_MASK)
            {
                return null;
            }
            else
            // PINGの場合はPONGを返す
            if(($entry_data['first_byte'] & 0x0f) === ParameterForConcurrentLoadBenchmark::CHAT_OPCODE_PING_MASK)
            {
                $p_param->setTempBuff(['recv_buff' => $entry_data]);
                return ProtocolForConcurrentLoadBenchmarkStatusEnum::PONG_CREATE->value;
            }
    ...
    protected function getRecvPongCreate()
    {
        return function(ParameterForConcurrentLoadBenchmark $p_param): ?string
        {
            ...
            $entry_data = $p_param->getTempBuff(['recv_buff']);
            $entry_data = $entry_data['recv_buff'];
            $b1 = ParameterForConcurrentLoadBenchmark::CHAT_FIN_BIT_MASK | (ParameterForConcurrentLoadBenchmark::CHAT_OPCODE_PONG_MASK & 0x0f);  // maskあり
            ...
            {
                $header = pack('CCn', $b1, ParameterForConcurrentLoadBenchmark::CHAT_PAYLOAD_LEN_CODE_2, $length);
            }
            //データ長８バイト
            else
            if($length >= 65536)
            {
                $header = pack('CCNN', $b1, ParameterForConcurrentLoadBenchmark::CHAT_PAYLOAD_LEN_CODE_8, $length);
            }
        
            $p_param->protocol()->setSendingData($header.$mask.$payload);

            return ProtocolForConcurrentLoadBenchmarkStatusEnum::PONG_SENDING->value;
        };
    }
    ...
    protected function getSendStart()
    {
        return function(ParameterForConcurrentLoadBenchmark $p_param): ?string
        {
            ...
            $b1 = ParameterForConcurrentLoadBenchmark::CHAT_FIN_BIT_MASK | (ParameterForConcurrentLoadBenchmark::CHAT_OPCODE_TEXT_MASK & 0x0f);  // maskあり
            $mask = random_bytes(4);
            ...
            return ProtocolForConcurrentLoadBenchmarkStatusEnum::SENDING->value;
        };
    }
    ...
    protected function getCloseStart()
    {
        return function(ParameterForConcurrentLoadBenchmark $p_param): ?string
        {
            ...
            $b1 = ParameterForConcurrentLoadBenchmark::CHAT_FIN_BIT_MASK | (ParameterForConcurrentLoadBenchmark::CHAT_OPCODE_CLOSE_MASK & 0x0f);  // maskあり
            $mask = random_bytes(4);
            $payload = $this->maskPayload($payload, $mask);
            $length = strlen($payload);
    ...
    protected function getAliveStart()
    {
        return function(ParameterForConcurrentLoadBenchmark $p_param): ?string
        {
            ...
            $b1 = ParameterForConcurrentLoadBenchmark::CHAT_FIN_BIT_MASK | (ParameterForConcurrentLoadBenchmark::CHAT_OPCODE_PING_MASK & 0x0f);  // maskあり
            $mask = random_bytes(4);
            $payload = $this->maskPayload($payload, $mask);
            $length = strlen($payload);
            ...
            if($length <= 125)
            {
                $header = pack('CC', $b1, $length | ~ParameterForConcurrentLoadBenchmark::CHAT_PAYLOAD_LEN_MASK);
            }
            // データ長２バイト
            else
            if($length > 125 && $length < 65536)
            {
                $header = pack('CCn', $b1, ParameterForConcurrentLoadBenchmark::CHAT_PAYLOAD_LEN_CODE_2 | ~ParameterForConcurrentLoadBenchmark::CHAT_PAYLOAD_LEN_MASK, $length);
            }
            //データ長８バイト
            else
            if($length >= 65536)
            {
                $header = pack('CCNN', $b1, ParameterForConcurrentLoadBenchmark::CHAT_PAYLOAD_LEN_CODE_8 | ~ParameterForConcurrentLoadBenchmark::CHAT_PAYLOAD_LEN_MASK, $length);
            }
