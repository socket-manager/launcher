...
use App\InitClass\InitForRawSocketStressTest;
use App\UnitParameter\ParameterForRawSocketStressTest;
use App\ProtocolUnits\ProtocolForRawSocketStressTest;
use App\CommandUnits\CommandForRawSocketStressTest;
 ...
 */
class TcpStressTest extends Console
{
    /**
     * @var string $identifer サーバー識別子
     */
    protected string $identifer = 'app:tcp-stress-test {samples} {sampling_interval?} {timeout?} {host?} {port?}';

    /**
     * @var string $description コマンド説明
     */
    protected string $description = 'TCP 耐久テスト: {samples} {sampling_interval?} {timeout?} {host?} {port?}';
    ...
    public function exec()
    {
        ...
        // UNITパラメータインスタンスの設定
        $param = new ParameterForRawSocketStressTest($samples, $this->sampling_interval);

        // SocketManagerの設定値初期設定
        $init = new InitForRawSocketStressTest($param, $this->port, 'TCP');
        $manager->setInitSocketManager($init);

        // プロトコルUNITの設定
        $entry = new ProtocolForRawSocketStressTest($this->payload, $this->host, $this->port, 'TCP');
        $manager->setProtocolUnits($entry);

        // コマンドUNITの設定
        $entry = new CommandForRawSocketStressTest($this->timeout);
